    ...
    user.Order.forEach((order) => {
      if (order.Status === "Paid") {
        ...
        if (existingDayIndex !== -1) {
          ...
          dailyPrices[existingDayIndex].totalPrice += order.Price
        } else {
    ...
    <Card align="center" direction="column" w="100%" {...rest}>
      <Flex justify="space-between" align="start" px="10px" pt="5px">
        ...
        <Flex align="center">
          ...
          <Text
            color={dailyTrafficPercentage >= 0 ? "green.500" : "red.500"}
            fontSize="sm"
            fontWeight="700"
          >
            {dailyTrafficPercentage >= 0 ? "+" : "-"}
            {Math.abs(dailyTrafficPercentage).toFixed(2)}%
          </Text>
        </Flex>
      </Flex>
